$(function () {
    ...
    cargarTablaProductoDisponible();
});

function cargarTablaProductoDisponible(estado = 1) {
    var datos = {"consultar_datos": "si_consulta", "estado": estado};//CREAMOS UN ARREGLO
    $.ajax({
        dataType: "json",
        method: "POST",
        //ubicacion del Controlador
        url: "../ControllerProductoDisponible",
        data: datos
    }).done(function (json) {
        Swal.close();
        if (json[0].resultado === "exito") {
            ...
            document.querySelector("#CuantosProductos").textContent = json[0].cuantos;
